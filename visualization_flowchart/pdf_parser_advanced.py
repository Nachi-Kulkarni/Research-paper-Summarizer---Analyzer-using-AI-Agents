...
def extract_structured_text(pdf_path: Path) -> Tuple[str, Dict[str, Any], List[Dict[str, Any]]]:
    """
    Extracts structured text content, metadata, and images from a PDF using layout analysis.
    ...
    Returns:
        A tuple containing:
          ...
          - A list of extracted images (each as dict with 'data' and 'metadata')
    ...
    try:
        ...
        return final_text, doc_metadata, extracted_images
...
def process_single_pdf_advanced(pdf_path: Union[str, Path], include_images: bool = True) -> Optional[dict]:
    """
    ...
    structured_text, metadata, images = extract_structured_text(pdf_path)

    if not structured_text and not images:
        parser_logger.warning(f"Extraction failed for {pdf_path.name}")
        return None
    ...
    try:
        if structured_text:
            text_path = save_structured_text(structured_text, pdf_path, output_dir=RAW_TEXT_DIR)
